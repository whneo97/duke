...
For seedu.duke.model.Duke, there are two events that we want to respond to, namely the user pressing `Enter` in the `TextField` and left-clicking the `Button`. These are the `onAction` event for the `TextField` and the `onMouseClicked` event for the `Button`.
...
For this tutorial, we have two images `DaUser.png` and `DaDuke.png` to represent the user avatar and seedu.duke.model.Duke's avatar respectively but you can use any image you want.
...
```java
public class seedu.duke.model.Duke extends Application {
    // ...
...
/**
 * Iteration 2:
 * Creates two dialog boxes, one echoing user input and the other containing seedu.duke.model.Duke's reply and then appends them to
...
private String getResponse(String input) {
    return "seedu.duke.model.Duke heard: " + input;
}
```
...
One additional benefit of defining a custom control is that we can add behavior specific to our `DialogBox`. Let’s add a method to flip a dialog box such that the image on the left to differentiate between user input and seedu.duke.model.Duke’s output.
...
Congratulations! 
You have successfully implemented a fully functional GUI for seedu.duke.model.Duke!  
