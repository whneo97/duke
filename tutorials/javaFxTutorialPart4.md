...
Let's return to seedu.duke.Duke and convert it to use FXML instead.
...
As part of the effort to separate the code handling seedu.duke.Duke's logic and UI, let's _refactor_ the UI-related code to its own class.
...
Let's create a new `Main` class as the bridge between the existing logic in `seedu.duke.Duke` and the UI in `MainWindow`.
