...
1. Create a folder `[project root]\text-validation-test`
1. Add a `runtest.bat` (if you are on Windows) or `runtest.sh` (if you are on a *nix OS) into the folder, containing the script below.<br>
  `runtest.bat`:
   ...
   javac  -cp ..\src -Xlint:none -d ..\bin ..\src\main\java\seedu.duke.model.Duke.java
   IF ERRORLEVEL 1 (
       echo ********** BUILD FAILURE **********
       exit /b 1
   )
   ...
   java -classpath ..\bin seedu.duke.model.Duke < input.txt > ACTUAL.TXT
   ...
   if ! javac -cp ../src -Xlint:none -d ../bin ../src/main/java/seedu.duke.model.Duke.java
   then
       echo "********** BUILD FAILURE **********"
       exit 1
   fi
